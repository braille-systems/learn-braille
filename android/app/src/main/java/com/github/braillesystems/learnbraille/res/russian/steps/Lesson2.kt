...
val LESSON_2_STEPS
    get() = listOf(
        Step(
            ...
            data = Info(
                """Некоторые символы Брайля обозначают как букву, так и цифру. Это буквы А, Б,
                    ...
                    |  уроке, из них получаются цифры 1, 2, 3 и так далее.""".trimMargin()
            )
        ),
        Step(
        ...
        Step(
            ...
            data = Info(
                """Откройте букварь на странице 13. Вверху слева рельефно-грфаическое
                    ...
                    | буква А точечным шрифтом.""".trimMargin()
            )
        ),
        Step(
        ...
        Step(
            ...
            data = Info(
                """Снова изучим страницу 13 в букваре. Под строкой с буквой А - 
                    |такая же с буквой Б.""".trimMargin()
            )
        ),
        Step(
        ...
        Step(
            ...
            data = Info(
                """Ознакомьтесь с буквой Ц на странице 13 букваря. 
                    |Строка с буквой Ц находится под строкой с буквой Б.""".trimMargin()
            )
        ),
        Step(
        ...
        Step(
            title = "В завершение первого урока",
            data = Info(
                """Поздравляем! Первый урок пройден. В следующем занятии мы узнаем, 
                    |как с помощью букв А, Б и Ц составить цифры 1, 2 и 3.""".trimMargin()
            )
        )
    )
